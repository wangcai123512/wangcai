...
namespace FMS.DAL
{
    public class DBHelper
    {
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    if (cols.Contains(pi.Name) && pi.CanWrite)
                    {
                        ...
                        if (value != DBNull.Value)
                        {
                            ...
                            {
                                pi.SetValue(t, Convert.ChangeType(value, Nullable.GetUnderlyingType(pi.PropertyType)), null);
                            }
        ...
        public string BulkCopyByDataTable(DataTable dtCopy,string strTableName)
        {
        ...
        public void AddDecimalPare(string name, SqlDbType type, ParameterDirection dir, byte Precision, byte Scale, object value)
        {
            SqlParameter para = new SqlParameter();
            para.ParameterName = name;
            para.SqlDbType = type;
        ...
        /// <returns>参数值</returns>
        public T GetParaValue<T>(string pname)
        {
            return (T)Cmd.Parameters[pname].Value;
        }

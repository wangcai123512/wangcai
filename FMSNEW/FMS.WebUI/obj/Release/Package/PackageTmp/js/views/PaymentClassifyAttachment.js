...
function showUploadFile(obj) {
    ...
    if (Dialog == "Editdialog") {
        var id = $("#Example_Edit #GUID").val();
    } else if (Dialog == "IncomeAdd") {
        var id = $("#Example_IncomeAdd #GUID").val();
    } else if (Dialog == "AddDcSpending") {
        var id = $("#Example_IncomeAdd #GUID").val();
    }
        showUploadFiles(id);
    }
...
function showUploadFiles(id) {
    ...
    $('#showhaveuploaded').bootstrapTable({
        ...
        columns: [[
            ...
            { field: 'FileName', title: '附件名称', align: 'center', width: '400px', remoteSort: true, },
...
var UploadedLinkHandle = function (value, row, index) {
    var link4 = " <a class='' href='/InternalAPI/DownLoadFile?fileID=" + value + "'>下载</a> ";
    var link3 = " <a class='' onclick='UploadedUpdateClick(\"" + value + "\")' style='cursor:pointer'>编辑</a> ";
    var link1 = " <a class='' onclick='UploadedDelClick(\"" + value + "\")' style='cursor:pointer'>删除</a> ";
    return link4+link3 + link1;
};
////下载附件
...
//};
//编辑附件附件
function UploadedUpdateClick(id) {
    $('#showhaveuploadedModal').modal('hide');
    var row = $('#showhaveuploaded').bootstrapTable('getRowByUniqueId', id);
    //名称
    $("#edithaveuploadedModal #TempFileName").val(row.FileName);
    $("#edithaveuploadedModal #FR_GUID").val(row.FR_GUID);
    //id
    $("#edithaveuploadedModal #GUID").val(id);
    $('#edithaveuploadedModal').modal({ show: true, backdrop: 'static' });
    $(".boxed-layout").css("padding-right", "0px");
}
//编辑model框提交
function EditUpload() {
    var EditGuid = $("#edithaveuploadedModal #FR_GUID").val();
    //对编辑提交的名称进行判断
    var TempFileName = $("#edithaveuploadedModal #TempFileName").val();
    if (TempFileName == "") {
        alert("请输入文件名称");
        error = true;
        return false;
    ...
    $.ajax({
        cache: true,
        type: "POST",
        url: "/InternalAPI/UpdHaveFileUpload",
        data: $('#D_Form').serialize(),
        async: false,
        onSubmit: function () {
            return $("#cmxform").form('validate');
        },
        ...
        success: function (data) {
            ...
            alert(res.Msg);
            $('#edithaveuploadedModal').modal('hide');
            showUploadFiles(EditGuid);
        }
    });
...
function ShowFirstUpLoadDialog(obj) {
    var Dialog = obj;
    if (Dialog == "Editdialog") {
        var GUID = $("#Example_Edit #GUID").val();
    } else if (Dialog == "IncomeAdd") {
        var GUID = $("#Example_IncomeAdd #GUID").val();
    } else if (Dialog == "AddDcSpending") {
        var GUID = $("#Example_IncomeAdd #GUID").val();
    }
...
function ShowUpLoadDialogs(GUID) {
    ...
    $("#qrcode").empty();
    var id = $("#id").val();
   var urladdress = "http://localhost:41955/UploadAttachment/UploadAttachment?belongid=" + GUID + "&tamp=" + timestamp + "&number=" + number;

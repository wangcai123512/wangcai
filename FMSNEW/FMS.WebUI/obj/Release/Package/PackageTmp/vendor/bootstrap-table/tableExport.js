  ...
  $.fn.extend({
    tableExport: function (options) {
      ...
      if (defaults.type == 'csv' || defaults.type == 'txt') {
        ...
        rowIndex = 0;
        trDatahead = "";
          //在table前面加标题等2018/0730改
            if (defaults.title == '资产负债表') {
                trDatahead_one = '<thead><tr><th colspan="8">' + defaults.title + '</th></tr></thead>';
                trDatahead_two = '<thead><tr><th colspan="2">公司:' + defaults.company + '</th><th colspan="4">' + defaults.midtitle + '</th><th colspan="2">单位:元</th></tr></thead>';
                trDatahead = trDatahead_one + trDatahead_two;
            } else if (defaults.title == '利润表') {
                trDatahead_one = '<thead><tr><th colspan="4">' + defaults.title + '</th></tr></thead>';
                trDatahead_two = '<thead><tr><th>公司:' + defaults.company + '</th><th>' + defaults.midtitle + '</th><th colspan="2">单位:元</th></tr></thead>';
                trDatahead = trDatahead_one + trDatahead_two;
            } else if (defaults.title == '现金流量表') {
                trDatahead_one = '<thead><tr><th colspan="4">' + defaults.title + '</th></tr></thead>';
                trDatahead_two = '<thead><tr><th>公司:' + defaults.company + '</th><th>' + defaults.midtitle + '</th><th colspan="2">单位:元</th></tr></thead>';
                trDatahead = trDatahead_one + trDatahead_two;
            }

        var docData = '<table>' + trDatahead + '<thead>';
        ...
        if (defaults.jspdf.autotable === false) {
          ...
          teOptions.doc = new jsPDF('p', 'pt');
          teOptions.doc.addFont('simhei.ttf', 'simhei', 'normal');
          teOptions.doc.setFont('simhei')
          //teOptions.doc.text(15, 15, '你好吗孩子我和好不用谢');
          //teOptions.doc.save('aaaaa.pdf');
          $(el).filter(function() {
              ...
              atOptions.beforePageContent = function (data) {
                if (data.pageCount == 1) {
                  ...
                  all.forEach(function (row) {
                      if (row.height > 0) {
                          if (!row.styles) {
                              row.height += (2 - FONT_ROW_RATIO) / 2 * 8;
                              data.table.height += (2 - FONT_ROW_RATIO) / 2 * 8;
                          } else {
            ...
            atOptions.styles.font = "simhei";
            //atOptions.headerStyles.font = "simhei";
            teOptions.doc.autoTable(teOptions.columns, teOptions.rows, atOptions);
      ...
      function jsPdfOutput(doc) {
        ...
        try {
            ...
            doc.save(defaults.fileName + '.pdf');

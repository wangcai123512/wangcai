...
namespace FMS.BLL
{
    ...
    public class IndirectMaterialPurchasingQueryController:UserController
    {
        public IndirectMaterialPurchasingQueryController()
            : base("IndirectMaterialPurchasingQuery")
        {}
        ...
        /// </summary>
        /// <returns></returns>
        public string GetIndirectMaterialPurchasingList(string dateBegin, string dateEnd, string customer, string Type, string TypeSub, string MaterielManage, string state = null, int pageIndex = 1, int pageSize = 10, string business_GUID = null, string subBusiness_GUID = null)
        {
            int count = 0;
            string C_GUID = Session["CurrentCompanyGuid"].ToString(); 
            List<T_AIDRecord> Record = new List<T_AIDRecord>();
            Record = new AIDSvc().GetIndirectMaterialPurchasingList(C_GUID, pageIndex, -1, out count,
        ...
        ///获取已转售列表数据
        /// </summary>
        public string GetIndirectMaterialResaleValuePurchasingList(string id, string dateBegin, string dateEnd, string customer, string grp, string state = null, int pageIndex = 1, int pageSize = 10)
        {
            int count = 0;

            string C_GUID = Session["CurrentCompanyGuid"].ToString();
            List<T_IERecord> Record = new List<T_IERecord>();
            Record = new IESvc().GetIndirectMaterialResaleValuePurchasingList(id, C_GUID, pageIndex, -1, out count,
        ...
        /// <param name="state"></param>
        /// <returns></returns>
        public string UpdIndirectMaterialPurchaseRecordState(string id, string state)
        {
            bool result = new AIDSvc().UpdAIDState(id, state);
            string msg = string.Empty;
            if (result)

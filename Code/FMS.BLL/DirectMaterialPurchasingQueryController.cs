...
namespace FMS.BLL
{
    ...
    public class DirectMaterialPurchasingQueryController:UserController
    {
        public DirectMaterialPurchasingQueryController()
            : base("DirectMaterialPurchasing_Query")
        {}
        ...
        /// </summary>
        /// <returns></returns>
        public string GetDirectMaterialPurchasingList(string page, string rows, string dateBegin, string dateEnd, string customer, string grp, string state = null)
        {
            int count = 0;
            string C_GUID = Session["CurrentCompany"].ToString();
            ...
            Record = new AIDSvc().GetDirectMaterialPurchasingList(C_GUID, int.Parse(page), int.Parse(rows), out count,
        ...
        {
            bool result = new AIDSvc().UpdAIDState("D", id, state);
            string msg = string.Empty;
            if (result)
            {
                msg = General.Resource.Common.Success;

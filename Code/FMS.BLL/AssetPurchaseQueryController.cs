...
namespace FMS.BLL
{
    ...
    public class AssetPurchaseQueryController:UserController
    {
        ...
        /// </summary>
        /// <returns></returns>
        public string GetAssetPurchaseList(string page, string rows, string dateBegin, string dateEnd, string customer, string grp, string state = null)
        {
            int count = 0;
            string C_GUID = Session["CurrentCompany"].ToString();
            ...
            Record = new AIDSvc().GetAssetPurchaseList(C_GUID, int.Parse(page), int.Parse(rows), out count,

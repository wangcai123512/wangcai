//查询净现金流
function RevenueCollectionQuery() {
    //查询汇总净现金流账
        $('#RevenueCollectionValue').bootstrapTable('destroy');
        $('#RevenueCollectionValue').bootstrapTable({
            //查询需要修改
            url: '/RevenueCollectionRecord/GetRevenueCollectionRecordList',
            method: 'get',//请求方式（*）
            pageSize: 10,//每页的记录行数（*）
...
function queryParams(params) {
    var temp = {   //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        ...
        BA_GUID: $('#RevenueCollection #CustomerList').val(),
    };
    return temp;
}
//导出
function RevenueCollectionExport() {
    $('#RevenueCollectionValue').tableExport({
        type: 'excel',
        fileName: '收入外收款',
        escape: false
    });

...
namespace FMS.BLL
{
    ...
    public class IncomeQueryController:UserController
    {
        ...
        public string GetIncomeList(string page, string rows, string dateBegin, string dateEnd, string customer, string state, string incomeGrp)
        {
            ...
            List<T_IERecord> recs = new IESvc().GetAllIncomeList(Session["CurrentCompany"].ToString(),int.Parse(page),int.Parse(rows),out count,
        ...
        public string GetReceivableList(string page, string rows)   
        {
             ...
             StringBuilder strJson = new StringBuilder();
             List<T_IERecord> recs = new IESvc().GetReceivableList(Session["CurrentCompany"].ToString(),int.Parse(page), int.Parse(rows), out count);
             strJson.AppendFormat(strFormatter, count, new JavaScriptSerializer().Serialize(recs));
             return strJson.ToString();
        }
        ...
        {
            int count = 0;
            List<T_IERecord> ds = new IESvc().GetAllIncomeList(Session["CurrentCompany"].ToString(),-1,-1, out count,
